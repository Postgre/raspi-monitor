...
# in t1.txt anmd t2.txt, and send to VPS server
# at 199.188.100.82 where main web page now resides
#
...
#
# Addition on 2/21/2014:
# Also, send via UDP to VPS 199.188.100.82:1024
...
while True:
	try:
		...
		ti = time.time()
		sample_time = time.localtime(ti)

		#
		# Update server via UDP at least every 1/2 hour
		#
		if abs( last_t1 - t1) > delta_t  or abs( last_t2 - t2) > delta_t \
			or  ti - tlast > 30 * 60:
		
			print "Termperature  database update starting at " + str(ti) + " seconds"
			tlast = ti
			sensor_data.send_dps([[ "t1", str(ti), t1], ["t2", str(ti), t2]])
